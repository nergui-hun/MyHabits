...
public final class Habit: Codable, Identifiable {
    ...
    public init(name: String, date: Date, trackDates: [Date] = [], color: Color) {
        self.name = name
        self.date = date
...
public final class HabitsStore: ObservableObject {
    ...
    public func save() {
        do {
            ...
            userDefaults.removeObject(forKey: "habits")
    ...
    public func editHabit(habit: Habit, newData: Habit) {
        guard let index = habits.firstIndex(of: habit) else { return }
        habits[index].name.removeAll(keepingCapacity: true)
        habits[index].name = newData.name
        habits[index].color = newData.color
        habits[index].date = newData.date
        save()
    }
    ...
    public func untrack(_ habit: Habit) {
        guard let index = habits.firstIndex(of: habit) else { return }
        if habits[index].isAlreadyTakenToday {
            habits[index].trackDates.removeLast()
            save()
        }
    }
